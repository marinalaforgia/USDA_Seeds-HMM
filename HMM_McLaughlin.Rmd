...
calculateQZ = function(param) # Qz transition matrix
{
  ...
  result[3,1] = (1 - param$c)*(1 - param$s)*(1 - param$r) # If r = 1, then this will always be zero. We assume that if a seed germinates and flowers it will produce a seed and if any seed is produced it goes into the seed bank (can't go from PP to AA), so why write out these other params?
  result[,2] = (1 - result[,1])*(1 - param$g) #[1,2] = (1-g)*c; [2,2] = (1-g)(1-(1-c)(1-s)); [3,2] = (1-g)
  result[,3] = (1 - result[,1])*param$g #[1,3] = gc; [2,3] = g(1-(1-c)(1-s)); [3,3] = g
  return(result)
}
```
...
calculatePeq = function(Q)
{
  ...
  #We look for an eigenvector whose components have the same sign and we renormalize it so that the sum makes 1 (this note confuses me)
...
for(i in unique(mcl$Species_Name)) {
  ...
  if(nrow(tmp) > 3) species.list[[i]] <- tmp # for now I got rid of species that were present in less than 8 sites throughout the entire time series, 167 species without down to 103, which is a lot, check with POC & Roger about a good cut-off
}
# 
...
surv <- read.csv("/Users/Marina/Documents/USDA-PostDoc/Projects/Sonoran-Desert/SD_Seed-survival.csv") # try to calculate fall*winter and new seeds summer survival and fall winter old seeds 
