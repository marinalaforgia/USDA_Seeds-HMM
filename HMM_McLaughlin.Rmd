...
This sets up a 2x2 matrix of seed bank transitions - I'm a little confused on this on what these values represent/how they are being used
...
* (1,1): 1-c -> probability that plants don't colonize (so what is there is there)
* (1,2): c -> colonization probability (probability that they do colonize)
* (2,1): (1-c)x(1-s)x(1-gr) -> the probability that plants dont colonize, don't survive, and don't germinate/produce seeds
* (2,2): 1 - ((1-c)x(1-s)x(1-gr)) -> the probability that seeds colonize, survive, and germinate?
...
calculateQY = function(param) # calculate QY is a function of param; QY transition matrix 
{
  ...
  return(result) # I'm still unclear on what this matrix is exactly, it's seed bank transitions but there is something I am missing
}
...
calculateQZ = function(param) # Qz transition matrix
{
  ...
  result[1,1] = 1 - param$c # probably seeds don't colonize
  result[2,1] = (1 - param$c)*(1 - param$s) # probability seeds don't colonize, and seeds that are there don't survive
  result[3,1] = (1 - param$c)*(1 - param$s)*(1 - param$r) # probability seeds dont colonize, seeds that are there don't survive and... wait what's r?
...
Is this the left eigenvector? the reproductive rate?
...
Still not sure what PZeq and PYeq are
...
makeParametersCalculations = function(param)
{
  ...
  param$PZeq = calculatePeq(param$QZ) # calculate the invariant of QZ transition matrix; equilibrium seed production?
  param$PYeq = param$PZeq[2] + param$PZeq[3] # Is this the invariant of Y? 
...
# Apply to McLaughlin 80 Site dataset

* Do I need to change the trueParams for each species? or are these just starting values?
...
# do I need to change these for my species?
...
# Notes
# Site version (the version specified here) took about 20 minutes or so
# Also tried it on individual quads instead of sites, took about 12 hours
...
Look at correlation between s and c!

```{r, echo = F}
library(ggplot2)
mcl.df <- readRDS("POC_HMM_McL.RDS")
#mcl.df <- readRDS("POC_HMM_McL_quad.RDS")
...
# Apply to Sonoran Desert dataset
...
# Compare Desert and Grassland trade-off
